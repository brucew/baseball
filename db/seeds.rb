...
Dir.glob('db/seed_data/*statistics.xml') do |stat_file|
  document = Nokogiri::XML(File.open stat_file, File::RDONLY)
      ...
      league_node.xpath('./DIVISION').each do |division_node|
        ...
        division_node.xpath('./TEAM').each do |team_node|
          ...
          team_node.xpath('./PLAYER').each do |player_node|
            ...
            Player.create(attrs.merge(season: season, league: league, division: division, team: team))
          end
